...
import {
  ShoppingCart, Plus, Search, Trash2, Printer, X, Eye, Loader2, ArrowLeft,
} from 'lucide-react'
import useExpenseStore from '@/store/useExpenseStore'
import {
  fetchPurchaseBills, fetchPurchaseBill, createPurchaseBill, deletePurchaseBill,
  type PurchaseBill, type PurchaseFormData,
} from '@/lib/api'
import PurchaseBillDocument from '@/components/PurchaseBillDocument'
...
export default function PurchaseBills() {
  ...
  const [preview, setPreview] = useState<PurchaseBill | null>(null)

  const load = async () => {
    setLoading(true)
    try {
      setBills(await fetchPurchaseBills(search || undefined))
    } catch {
      addToast('error', 'Failed to load purchase bills')
    } finally {
      setLoading(false)
    }
  }
  ...
  if (mode === 'create') {
    return <CreateBill onDone={() => setMode('list')} onPreview={(b) => { setMode('list'); setPreview(b) }} />
  }
    ...
    <div className="animate-fade-in">
      <div className="flex flex-col gap-4 mb-6 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-surface-900 dark:text-white">Purchase Bills</h2>
            <p className="text-sm text-surface-500 dark:text-surface-400">Record bills from vendors</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
      ...
      <div className="rounded-2xl bg-white dark:bg-surface-800 shadow-sm border border-surface-100 dark:border-surface-700 overflow-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            <ShoppingCart className="w-12 h-12 text-surface-300 dark:text-surface-600 mb-3" />
            <p className="text-surface-500 dark:text-surface-400 font-medium">No purchase bills yet</p>
              ...
              <tbody>
                {bills.map((b) => (
                  <tr key={b.id} className="border-b border-surface-100 dark:border-surface-700/50 hover:bg-surface-50/50 dark:hover:bg-surface-700/30 transition-colors">
                    <td className="p-3 font-semibold text-surface-900 dark:text-white">{b.bill_number}</td>
                    <td className="p-3 text-surface-700 dark:text-surface-300">{b.vendor_name}</td>
                    ...
                    <td className="p-3">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <DeleteButton id={b.id} number={b.bill_number} onDeleted={() => setBills((p) => p.filter((x) => x.id !== b.id))} onToast={addToast} />
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      {preview && <PreviewModal billId={preview.id} onClose={() => setPreview(null)} />}
    </div>
  )
}
  ...
  return (
    <div className="flex items-center gap-1">
      <button
        onClick={async () => {
          setBusy(true)
          try { await deletePurchaseBill(id); onToast('success', `Deleted ${number}`); onDeleted() }
...
function PreviewModal({ billId, onClose }: { billId: number; onClose: () => void }) {
  const [bill, setBill] = useState<PurchaseBill | null>(null)
  useEffect(() => {
    fetchPurchaseBill(billId).then(setBill).catch(() => {})
  }, [billId])
...
// ---------------------------------------------------------------------------
function CreateBill({ onDone, onPreview }: { onDone: () => void; onPreview: (b: PurchaseBill) => void }) {
  const addToast = useExpenseStore((s) => s.addToast)
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      const created = await createPurchaseBill(payload)
      addToast('success', `Purchase bill ${created.bill_number} created`)
      onPreview(created)
    } catch {
    ...
    <div className="animate-fade-in">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-xl font-bold text-surface-900 dark:text-white">New Purchase Bill</h2>
        </div>
      ...
      <div className="rounded-2xl bg-white dark:bg-surface-800 shadow-sm border border-surface-100 dark:border-surface-700 p-5 mb-5 grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div><label className={labelCls}>Vendor invoice number</label><input className={inputCls} value={vendorInvoiceNumber} onChange={(e) => setVendorInvoiceNumber(e.target.value)} placeholder="e.g. UM/25-26/26307" /></div>
        ...
        <div className="md:col-span-2"><label className={labelCls}>Bill number (optional — auto if blank)</label><input className={inputCls} value={billNumber} onChange={(e) => setBillNumber(e.target.value)} placeholder="e.g. RT25-26/PUR-5073" /></div>
      </div>
