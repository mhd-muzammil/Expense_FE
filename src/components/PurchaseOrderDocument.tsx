...
// Pixel-styled printable PURCHASE ORDER matching the reference PDF. Orange accent,
// GST table, vendor + ship-to, "Valid Until" date, totals with amount in words.
...
export default function PurchaseOrderDocument({ order }: { order: PurchaseOrder }) {
  const shipName = order.ship_to_name || COMPANY.name
  const shipAddr = order.ship_to_address || COMPANY.address
    ...
    <div className="invoice-doc" style={styles.page}>
      ...
      <div style={styles.headerRow}>
        ...
        <div style={{ textAlign: 'center', flex: 1.2 }}>
          <div style={{ fontSize: 20, fontWeight: 700, color: '#374151', letterSpacing: 0.5 }}>PURCHASE ORDER</div>
        </div>
        <div style={{ textAlign: 'right', flex: 1 }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: '#1f2937' }}>{order.order_number}</div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 16, marginTop: 14 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={styles.amountBar}>
            ...
            <span>INR {fmt(order.grand_total)}</span>
          </div>
          <table style={{ width: '100%', fontSize: 10, marginTop: 8, color: '#374151' }}>
            <tbody>
              <tr><td style={styles.metaLabel}>Issue Date:</td><td style={styles.metaVal}>{fmtDate(order.issue_date)}</td></tr>
              <tr><td style={styles.metaLabel}>Valid Until:</td><td style={styles.metaVal}>{fmtDate(order.valid_until)}</td></tr>
              <tr><td style={styles.metaLabel}>Place of Supply:</td><td style={styles.metaVal}>{order.place_of_supply || '—'}</td></tr>
            </tbody>
          </table>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 24, marginTop: 18, fontSize: 9.5, color: '#374151', lineHeight: 1.5 }}>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontWeight: 700, fontSize: 11, color: '#111827' }}>{order.vendor_name}</div>
          {order.vendor_address && <div>{order.vendor_address}</div>}
          {order.vendor_phone && <div>{order.vendor_phone}</div>}
          {order.vendor_gstin && <div><strong>GSTIN:</strong> {order.vendor_gstin}{order.vendor_pan ? <>&nbsp;&nbsp;<strong>PAN:</strong> {order.vendor_pan}</> : null}</div>}
          {!order.vendor_gstin && order.vendor_pan && <div><strong>PAN:</strong> {order.vendor_pan}</div>}
        </div>
        <div style={{ flex: 1 }}>
      ...
      <table style={styles.itemTable}>
        ...
        <tfoot>
          <tr style={{ borderTop: '2px solid #d1d5db', fontWeight: 700 }}>
            ...
            <td style={{ ...styles.td, textAlign: 'right' }}>{fmt(order.taxable_total)}</td>
            <td style={{ ...styles.td, textAlign: 'right' }}>{fmt(order.cgst_total)}</td>
            <td style={{ ...styles.td, textAlign: 'right' }}>{fmt(order.sgst_total)}</td>
            <td style={{ ...styles.td, textAlign: 'right' }}>{fmt(order.grand_total_raw)}</td>
          </tr>
        </tfoot>
      </table>
        ...
        <table style={{ fontSize: 10, color: '#374151' }}>
          <tbody>
            <tr>
              ...
              <td style={styles.totVal}>INR {fmt(order.taxable_total)}</td>
            </tr>
            {parseFloat(order.rounded_off) !== 0 && (
              <tr>
                <td style={styles.totLabel}>Rounded Off</td>
                <td style={styles.totVal}>
                  {parseFloat(order.rounded_off) < 0 ? '(-) ' : '(+) '}INR {fmt(Math.abs(parseFloat(order.rounded_off)))}
                </td>
              </tr>
            )}
            <tr>
              <td style={{ ...styles.totLabel, fontWeight: 700, color: '#111827' }}>Total Value (in figure)</td>
              <td style={{ ...styles.totVal, fontWeight: 700, color: '#111827' }}>INR {fmt(order.grand_total).replace(/\.00$/, '')}</td>
            </tr>
            <tr>
              <td style={styles.totLabel}>Total Value (in words)</td>
              <td style={{ ...styles.totVal, fontWeight: 700 }}>INR {order.amount_in_words} Only</td>
            </tr>
          </tbody>
        </table>
      </div>
        ...
        <div style={{ fontSize: 9, color: '#6b7280', maxWidth: '55%' }}>
          {order.terms && (
            <>
              ...
              <div style={{ whiteSpace: 'pre-line' }}>{order.terms}</div>
            </>
          )}
        </div>
